...
import { getContactSchema } from "@/utils/validation";
import client from "@/services/api-client";
import { IP_CHECK_URL, ERROR_MESSAGES, GA_ID } from "@/utils/app-constants";
import { logger } from "@/utils/logger";
import { toast } from "sonner";
import api from "@/services";
import { pushGtmEvent } from "@/utils/helper";

/**
 * Custom hook to handle general contact form logic, state, and validation.
 * @param {Object} props - Hook properties.
 * @param {Function} props.setContactOpen - State setter to close the dialog.
 */
export const useContactForm = ({ setContactOpen }) => {
  const { t, i18n } = useTranslation();
  const lang = i18n.language;

  // Helper to extract tracking data
  ...
  const onSubmit = async (values) => {
    try {
      const isHe = lang === "he" || window.location.pathname.includes("dashboard-he");
      const formId = isHe ? "5311" : "1655";
      ...
      const response = isHe
        ? await api.contact.postHe(formData)
        : await api.contact.postEn(formData);
